...
Bellman.listen() {
	...
	var soundMachine = new Listener(new Sonos(process.env.SONOS_HOST || '192.168.0.0'));
		...
		Bellman.addService('/MediaRenderer/AVTransport/Event', function(error, sid) {
			if (error) throw err;
		...
		Bellman.on('serviceEvent', function(endpoint, sid, data) {
...
Bellman.checkForNewTrack(currentlyPlaying) {
...
Bellman.sendToSlack(trackMetadata) {
